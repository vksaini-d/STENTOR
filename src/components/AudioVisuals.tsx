...
export function AudioLevelMeter({ level, className }: { level: number; className?: string }) {
 return (
 <div className={cn("w-full h-1.5 bg-[--color-surface-raised] rounded-full overflow-hidden", className)}>
 <div
...
export function AudioRing({ level, className }: { level: number; className?: string }) {
 ...
 <div
 className={cn(
 "absolute inset-0 rounded-full bg-[--color-signal] transition-all duration-100 ease-out pointer-events-none",
 className,
 )}
 style={{
...
export function StatusPill({
 ...
 const colors = {
 default: 'bg-[--color-surface-raised] border-[--color-surface] text-[--color-text-secondary]',
 success: 'bg-[--color-signal]/10 border-[--color-signal]/30 text-[--color-signal]',
...
export function StatusDot({ active, className }: { active: boolean; className?: string }) {
 ...
 active ? "bg-[--color-signal] animate-pulse" : "bg-slate-500",
 className,
 )} />
 );
}
